 ...
 * title comes from the deepest matching route's `handle.title` and is also
 * the document title.
 */
export function AppLayout() {
  const matches = useMatches()
  const title = [...matches].reverse().map((match) => match.handle).find(isAppRouteHandle)?.title ?? 'QueueFlow'
